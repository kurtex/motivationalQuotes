...
import ScheduledPost, { IScheduledPost } from "@/app/lib/database/models/ScheduledPost"; // Import IScheduledPost
    ...
    if (scheduleType === 'custom' && intervalValue && intervalUnit) {
        ...
        while (currentIntervalDate.getTime() < now.getTime()) {
            if (intervalUnit === 'hours') {
                currentIntervalDate.setHours(currentIntervalDate.getHours() + intervalValue);
            } else if (intervalUnit === 'days') {
                currentIntervalDate.setDate(currentIntervalDate.getDate() + intervalValue);
            } else if (intervalUnit === 'weeks') {
                currentIntervalDate.setDate(currentIntervalDate.getDate() + (intervalValue * 7));
            }
        }
...
export async function POST(req: NextRequest) {
    ...
    try {
        const { scheduleType, intervalValue, intervalUnit, timeOfDay } = await req.json();

        if (!scheduleType || !timeOfDay) {
            return NextResponse.json({ error: "Schedule type and time of day are required" }, { status: 400 });
        }
        if (scheduleType === 'custom' && (!intervalValue || !intervalUnit)) {
            return NextResponse.json({ error: "Interval value and unit are required for custom schedule" }, { status: 400 });
        }
        ...
        if (!tokenDoc) {
            return NextResponse.json({ error: "Unauthorized: Invalid Threads token" }, { status: 401 });
        }
        ...
        const nextScheduledAt = calculateNextScheduledAt(scheduleType, timeOfDay, intervalValue, intervalUnit);
        ...
        return NextResponse.json({ message: "Schedule updated successfully", scheduleId: scheduledPost._id }, { status: 200 });
    } catch (error: any) {
