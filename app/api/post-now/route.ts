...
export async function POST(req: NextRequest) {
    try {
        ...
        return NextResponse.json({ message: "Quote posted successfully", quote: generatedQuote }, { status: 200 });
    } catch (error: any) {
        console.error("Error posting quote immediately:", error);
        return NextResponse.json({ error: error.message || "Failed to post quote immediately" }, { status: 500 });
    }
}
