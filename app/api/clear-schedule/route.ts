...
export async function POST(req: NextRequest) { // Using POST for state change
    try {
        ...
        if (success) {
            ...
            return NextResponse.json({ error: "No active schedule found to clear" }, { status: 404 });
        }
    } catch (error: any) {
        console.error("Error clearing schedule:", error);
        return NextResponse.json({ error: error.message || "Failed to clear schedule" }, { status: 500 });
    }
}
