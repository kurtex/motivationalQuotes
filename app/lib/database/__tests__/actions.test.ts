...
describe('Database Actions', () => {
    ...
    describe('getMetaUserIdByThreadsAccessToken', () => {
        it('should return user_id for a valid token', async () => {
            ...
            expect(MockedToken.findOne).toHaveBeenCalledWith({ access_token: 'valid-token' });
        });

        it('should throw an error for an invalid token', async () => {
            MockedToken.findOne.mockResolvedValue(null);
            await expect(getMetaUserIdByThreadsAccessToken('invalid-token')).rejects.toThrow('Token not found');
        });
    });
    ...
    describe('saveGeminiQuote', () => {
        ...
        it('should throw error for duplicate hash', async () => {
            ...
            await expect(saveGeminiQuote('A new quote', 'access-token')).rejects.toThrow('Duplicate quote detected (hash)');
        });
        ...
        it('should throw error for near-duplicate embedding', async () => {
            ...
            await expect(saveGeminiQuote('A new quote', 'access-token')).rejects.toThrow('Near-duplicate quote detected (embedding)');
        });
    });
    ...
    describe('saveUniqueGeminiQuote', () => {
        ...
        it('should retry on duplicate hash error and succeed', async () => {
            mockGenerateContent.mockResolvedValueOnce('Duplicate quote').mockResolvedValueOnce('Unique quote');
        ...
        it('should throw after max retries', async () => {
            ...
            await expect(saveUniqueGeminiQuote('access-token', [], 3)).rejects.toThrow('No unique quote could be generated after 3 attempts');
